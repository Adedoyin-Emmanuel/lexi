...
import { AnalysisResult, Clause, Risk, Obligation, NegotiationSuggestion } from "../page";
    ...
    <div className="h-full flex flex-col">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col">
        <TabsList className="grid w-full grid-cols-5 mx-4 mt-4">
          ...
          <TabsTrigger value="obligations" className="text-xs">Obligations</TabsTrigger>
          <TabsTrigger value="negotiation" className="text-xs">Redlines</TabsTrigger>
        </TabsList>
        ...
        <ScrollArea className="flex-1 px-4 pb-4">
          ...
          <TabsContent value="risks" className="mt-4 space-y-3">
            {analysis.risks.length === 0 ? (
              <div className="text-center py-8 text-gray-500">
                ...
                <p className="text-sm">Your contract looks good!</p>
              </div>
            ) : (
          ...
          <TabsContent value="obligations" className="mt-4 space-y-3">
            {analysis.obligations.length === 0 ? (
              <div className="text-center py-8 text-gray-500">
                <p>No specific obligations found</p>
                <p className="text-sm">Check the contract terms for deadlines</p>
              </div>
            ) : (
          ...
          <TabsContent value="negotiation" className="mt-4 space-y-3">
            {analysis.negotiationSuggestions.length === 0 ? (
              <div className="text-center py-8 text-gray-500">
                <p>No negotiation suggestions</p>
                <p className="text-sm">Your contract terms are well-balanced</p>
              </div>
            ) : (
